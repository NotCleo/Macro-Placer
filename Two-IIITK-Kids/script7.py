...
def run_global_placement(
    ...
    *,
    ...
    device = _device()
    # V13-rev4: force deterministic execution so Phase α produces the same
    # starting basin across runs. Without this, FFT-Poisson + Adam on cudnn
    # exhibited ±0.5 proxy variance run-to-run (1.32 / 1.55 / 1.85 on ibm08
    # with same seed). The `warn_only=True` flag lets ops that have no
    # deterministic impl fall back without aborting; the cudnn flags + cuda
    # seed cover the GP's actual hot path.
    torch.manual_seed(seed)
    np.random.seed(seed)
    if device.type == "cuda":
        torch.cuda.manual_seed_all(seed)
    _prev_cudnn_det = torch.backends.cudnn.deterministic
